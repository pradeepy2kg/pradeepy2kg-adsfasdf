   ...
   * */
    public String login() {
        if (loginBD.login(userName, password)) {
            ...
            session.put(Constant.SESSION_USER_LANG, userPreferencesAction.getLanguage());
            session.put(Constant.SESSION_USER_NAME, userName);
            return "success";
        }
        return "error";
    }
    ...
    public String logout(){
        if(session.containsKey(Constant.SESSION_USER_NAME)){
            logger.debug("Inside logout : {} is going to logout.", session.get(Constant.SESSION_USER_NAME).toString());
            session.remove(Constant.SESSION_USER_NAME);
            return "success";
        }
        return "error";
    }
