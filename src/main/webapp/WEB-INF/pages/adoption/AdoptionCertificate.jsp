<%@ page import="java.util.Date" %>
<%@ taglib prefix="s" uri="/struts-tags" %>
<%@ taglib prefix="sx" uri="/struts-dojo-tags" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>

<div id="adoption-certificate-form-outer">
<form action="eprAdoptionReRegistration.do">
<table style="width: 100%; border:none; border-collapse:collapse; ">
    <col width="200px">
    <col width="400px">
    <col width="200px">
    <tbody>
    <tr>
        <td rowspan="3"></td>
        <td rowspan="2" align="center">
            <img src="<s:url value="../images/official-logo.png" />"
                 style="display: block; text-align: center;" width="80" height="100">
        </td>
        <td>සහතික පත්‍රයේ අංකය <br>சான்றிதழ் இல <br>Certificate Number
        </td>
    </tr>
    <tr>
        <td><s:label value="%{adotion.birthCertificateNumber}"/></td>
    </tr>
    <tr>
        <td align="center">ශ්‍රී ලංකා / ﻿இலங்கை / SRI LANKA <br>
            දරුකමට ගැනීම පිලිබඳ සහතිකය <br>
            பிறப்பு சான்றிதழ்﻿ <br>
            CERTIFICATE OF ADOPTION
        </td>
        <td></td>
    </tr>
    </tbody>
</table>

<table class="table_con_page_01" width="100%" cellspacing="0" style="margin-top:10px;float:left;>
    <col width="135px">
    <col>
    <col width="145px">
    <col>
    <tbody>
    <tr>
        <td width="175px" height="80px">දිස්ත්‍රික්කය<br>மாவட்டம் <br>District
        </td>
        <td width="300px"><s:label name="" value="%{adotion.}"/><br/>
            <s:label name="" value="%{adotion.}"/></td>
        <td width="200px">ප්‍රාදේශීය ලේකම් කොට්ඨාශය
            <br> பிரிவு
            <br> Divisional Secretariat
        </td>
        <td><s:label name="" value="%{adotion.}"/><br/>
            <s:label name="" value="%{adotion.}"/></td>
    </tr>
    <tr>
        <td width="200px" height="90px">ලියාපදිංචි කිරීමේ කොට්ඨාශය
            <br>பிரிவு
            <br>Registration Division
        </td>
        <td><s:label name="" value="%{}"/></td>
            <td>මුල් ලියාපදිංචියෙන් පසු වෙනස්කම්
                <br> நிறைவேற்றிய மாற்றங்கள்
                <br> Changes after first registration
            </td>
            <td><s:label name="" value="%{adotion.}"/></td>
    </tr>
    </tbody>
</table>

<table class="table_con_page_01" width="100%" cellspacing="0" style="margin-top:10px;float:left;">
    <col width="150px">
    <col width="160px">
    <col width="100px">
    <col width="130px">
    <col width="100px">
    <col>
    <tbody>
    <tr>
        <td height="80px"  >පුද්ගල අනන්‍යතා අංකය <br>தனிநபர்அடையாள எண் <br>Person Identification Number (PIN)
        </td>
        <td colspan="2"><s:label name="" value="%{#adotion.}"/></td>
        <td width="100px">උපන් දිනය <br>பிறந்த திகதி <br>Date of birth <br>YYYY-MM-DD
        </td>
        <td width="150px"><s:label name="" value="%{adotion.childBirthDate}"/></td>
        <td>ස්ත්‍රී පුරුෂ භාවය<br>பால் <br>Gender
        </td>
        <td width="150px">
            <s:label name="" value="%{adotion.childGender}"/><br/>
            <s:label name="" value="%{adotion.childGender}"/>
        </td>
    </tr>
    <tr>
        <td height="80px">දරුකමට හදාගැනීමේ උසාවි නියෝගය <br>
            in tamil <br>
            Adoption Order Issued by Court
        </td>
        <td width="150px">අධිකරණය <br>
            in tamil <br>
            Court
        </td >
        <td width="60px">
            <s:label value="%{adoption.court}"/>
        </td>
        <td width="100px">නියෝග දිනය <br>
            in tamil <br>
            Order Date
        </td>
        <td width="60px">
            <s:label value="%{adoption.orderIssuedDate}"/>
        </td>
        <td>නියෝග අංකය <br>
            in tamil <br>
            Order Number
        </td>
        <td width="150px">
            <s:label value="%{adoption.courtOrderNumber}"/>
        </td>
    </tr>

    <tr>
        <td height="120px">නම <br>பெயர் <br>Name
        </td>
        <td colspan="6" class="bc-name" style="font-size:12pt">
            <s:label name="" value="%{adotion.childNewName}"/>
        </td>
    </tr>
    <tr>
        <td height="110px">නම ඉංග්‍රීසි භාෂාවෙන් <br>ஆங்கிலத்தில் பெயர் <br> Name in English
        </td>
        <td colspan="6" class="bc-name" style="font-size:12pt">
            <s:label name="" cssStyle="text-transform: uppercase;" value="%{adotion.childNewName}"/>
        </td>
    </tr>
    <tr>
        <td height="120px">පියාගේ සම්පුර්ණ නම<br>தந்தையின்முழுப் பெயர் <br> Father's Full Name
        </td>
        <td colspan="6" class="bc-name" style="font-size:12pt">
            <s:label name="" value="%{adotion.}"/>
        </td>
    </tr>
    <tr>
        <td height="70px">පියාගේ අනන්‍යතා අංකය හෝ ජාතික
            හැඳුනුම්පත් අංකය <br>
            தந்தையின் அடையாள எண் <br>
            Father's PIN / NIC
        </td>
        <td colspan="2"><s:label name="" value="%{adotion.}"/></td>
        <td>පියාගේ රට <br>
            நாடு <br>
            Father's Country
        </td>
        <td colspan="3"><s:label name="" value="%{adotion.}"/><br/>
            <s:label name=""  value="%{adotion.}"/>
        </td>

    </tr>
    <tr>
        <td height="70px">පියාගේ ජාතිය <br>
            தந்தையின் இனம் <br>
            Father's Race
        </td>
        <td colspan="2"><s:label name="" value="%{adotion.}"/></td>
        <td>පියාගේ ගමන් බලපත්‍ර අංකය <br>
            கடவுச் சீட்டு <br>
            Father's Passport No.
        </td>
        <td colspan="3"><s:label name="" value="%{adotion.}"/><br/><s:label name=""
                                                                                   value="%{fatherRacePrintEn}"/>
        </td>

    </tr>
    <tr>
        <td height="120px"> මවගේ සම්පූර්ණ නම <br>
            தாயின் முழுப் பெயர் <br>
            Mother's Full Name
        </td>
        <td colspan="6" class="bc-name" style="font-size:12pt">
            <s:label name="" value="%{adotion.}"/>
        </td>
    </tr>
    <tr>
        <td height="70px">ම‌වගේ අනන්‍යතා අංකය හෝ ජාතික හැඳුනුම්පත් අංකය <br>
            தந்தையின் அடையாள எண் <br>
            Mother's PIN / NIC
        </td>
        <td colspan="2"><s:label name="" value="%{adotion.}"/></td>
        <td>ම‌වගේ රට <br>
            நாடு <br>
            Mother's Country
        </td>
        <td colspan="3"><s:label name="" value="%{adotion.}"/><br/>
            <s:label name="" value="%{adotion.}"/></td>
    </tr>
    <tr>
        <td height="70px">ම‌වගේ ජාතිය <br>
            தந்தையின் இனம் <br>
            Mother's Race
        </td>
        <td colspan="2"><s:label name="" value="%{adotion.}"/></td>
        <td>ම‌වගේ ගමන් බලපත්‍ර අංකය <br>
            கடவுச் சீட்டு <br>
            Mother's Passport No.
        </td>
        <td colspan="3"><s:label name="" value="%{adotion.}"/><br/>
            <s:label name="" value="%{adotion.}"/>
        </td>

    </tr>
    </tbody>
</table>

<table class="table_con_page_01" width="100%" cellspacing="0" style="margin-top:10px;float:left;>
    <col width="195px">
    <col width="215px">
    <col width="120px">
    <col>
    <tbody>
    <tr>
        <td height="70px">ලියාපදිංචි කළ දිනය<br>பதிவு செய்யப்பட்ட திகதி <br> Date of Registration
        </td>
        <td><s:label name="" value="%{adotion.}"/></td>
        <td>නිකුත් කළ දිනය<br>வழங்கிய திகதி <br> Date of Issue
        </td>
        <td width="250px"><s:label name="" value="%{adotion.}"/></td>
    </tr>
    <tr>
        <td colspan="2" height="120px">
            සහතික කරනු ලබන නිලධාරියා ගේ නම, තනතුර සහ අත්සන <br>
            சான்றிதழ் அளிக்கும் அதிகாரியின் பெயர், பதவி, கையொப்பம்<br>
            Name, Signature and Designation of certifying officer
        </td>
        <td colspan="2" style="font-size:10pt"><s:label name=""  value="%{adotion.}"/></td>
    </tr>
    <tr>
        <td colspan="2" height="30px">නිකුත් කළ ස්ථානය / வழங்கிய இடம் / Place of Issue
        </td>
        <td colspan="2">රෙජිස්ට්‍රාර් ජනරාල් කාර්යාලය - කොළඹ <br>
            Registrar Generals Department - Colombo
        </td>
    </tr>
    </tbody>
</table>

<p>(1941 අංක 24 දරන දරුකමට ගැනීම පිලිබඳ ආඥාපනතේ 11 වෙනි වගන්තිය යටතේ නිකුත් කරන ලදී) <br>
    in tamil <br>
    (Issued under Section 11 of the Adoption of Children Ordinance, No. 24 of 1941)

<p></p>

<div class="adoption-form-submit">
    <s:submit onclick="printPage();" value="%{getText('print.button')}" cssStyle="margin-top:10px;"/>
</div>
</form>
</div>
